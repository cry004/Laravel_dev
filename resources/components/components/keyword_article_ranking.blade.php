...
<div class="Heading section-spacing section-start-spacing">
    <h2 class="Heading-Text">{{ $tagName->text_content }}に関する記事人気ランキング</h2>
...
<div class="Entry">
    @if(!$article->is_tieup)
        <a class="rnkArticle-0{{$index+1}}" href="{{ $article->getDetailLinkURL() }}">
    @else
        <a class="rnkArticle-0{{$index+1}}" href="{{ AppUrl::to('tu/article-' . $article->id) }}">
    @endif
